...
from database import Account, Transaction, Stock, StockCache, Recommendation, StockOHLCVCache, TransactionCash, AccountDailyBalance
...
def run_migrations():
    ...
    if not has_legacy:
        print("[Migration] No legacy 'transactions' table found. Schema might already be updated.")
    ...
    print(f"[Migration] Backup complete: {len(accounts_data)} accounts, {len(transactions_data)} transactions, {len(stocks_data)} stocks.")
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    legacy_tables = ["account", "transactions", "stocks", "cache_stocks", "recommendations", "stock_ohlcv_cache"]
    ...
    try:
        ...
        for d in transactions_data:
            ...
            if not db.query(StockCache).filter_by(stock_code=d['code']).first():
                db.merge(StockCache(stock_code=d['code'], stock_name=d['name'], dt_cached=datetime.utcnow()))
                db.commit()
        ...
        for d in stocks_data:
            if not db.query(StockCache).filter_by(stock_code=d['code']).first():
                db.merge(StockCache(stock_code=d['code'], stock_name=d.get('name','알수없음'), dt_cached=datetime.utcnow()))
                db.commit()
