...
from database import Account, Stock, StockOHLCVCache, Transaction, TransactionCash, AccountDailyBalance, StockCache
...
def get_exact_trade_date_limits(target_period=60):
    ...
    try:
        ...
        df_market = krx_stock.get_market_ohlcv_by_date(safe_start_str, target_end_str, "KOSPI")
        if df_market is None or df_market.empty:
            raise ValueError("Empty df")
        actual_trade_dates = df_market.index.strftime("%Y%m%d").tolist()
    except Exception:
        try:
            df_market = krx_stock.get_market_ohlcv_by_date(safe_start_str, target_end_str, "005930")
    ...
    safe_fallback_start = (now - timedelta(days=int(target_period * 1.5))).strftime("%Y%m%d")
    ...
    for date, row in df.iterrows():
        ...
        if not existing:
            cache_entry = StockOHLCVCache(
                ...
                fluctuation_rate=float(row.get("등락률", 0.0)) if has_fluctuation else 0.0
            )
            db.add(cache_entry)
    db.commit()
...
def sync_ohlcv_cache(db: Session, stock_code: str, start_date: str = None, end_date: str = None):
    ...
    try:
        last_cache = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == stock_code).order_by(StockOHLCVCache.trade_date.desc()).first()
        first_cache = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == stock_code).order_by(StockOHLCVCache.trade_date.asc()).first()
...
def calculate_stock_balance(transactions, target_stock_code, acc_cd):
    ...
    sorted_tx = sorted(
        [t for t in transactions if t.stock_code == target_stock_code and t.acc_cd == acc_cd and t.dt_deleted is None],
...
def get_enriched_accounts_data(db: Session) -> dict:
    db_accounts = db.query(Account).filter(Account.dt_deleted.is_(None)).order_by(Account.acc_order.asc()).all()
    db_stocks = db.query(Stock).all()
    all_transactions = db.query(Transaction).filter(Transaction.dt_deleted.is_(None)).all()
    ...
    for stock in db_stocks:
        ...
        latest_cache = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == code).order_by(StockOHLCVCache.trade_date.desc()).first()
        if latest_cache:
    ...
    for acc in db_accounts:
        ...
        stocks_eval = int(round(sum(s["quantity"] * s["current_price"] for s in stocks_list)))
        total_eval = int(round(stocks_eval + acc.cash_balance))
        total_purchase = int(round(stocks_purchase + acc.cash_balance))
        profit_rate = round(((total_eval - total_purchase) / total_purchase) * 100, 2) if total_purchase > 0 else 0.0
...
def recalculate_portfolio_for_account(db: Session, acc_cd: str):
    ...
    if not account:
        raise HTTPException(status_code=404, detail=f"Account with code '{acc_cd}' not found.")

    cash_balance = float(account.initial_cash or 0.0)
    holdings = {}

    # Load and combine all valid transactions
    stock_txs = db.query(Transaction).filter(Transaction.acc_cd == acc_cd, Transaction.dt_deleted.is_(None)).all()
    cash_txs = db.query(TransactionCash).filter(TransactionCash.acc_cd == acc_cd, TransactionCash.dt_deleted.is_(None)).all()
    ...
    for event in events:
        if event["type"] == "CASH":
            ctx = event["data"]
            if ctx.cash_type in [CashType.DEPOSIT, CashType.INTEREST, CashType.DIVIDEND]:
            ...
            if tx.trade_type == TradeType.BUY:
                ...
                if code not in holdings or holdings[code]["quantity"] < qty:
                    raise HTTPException(status_code=400, detail=f"Insufficient stock holdings for SELL transaction ({code}).")
    ...
    for code, info in holdings.items():
        ...
        latest_cache = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == code).order_by(StockOHLCVCache.trade_date.desc()).first()
        current_p = latest_cache.close_price if latest_cache else int(round(info["avg_price"]))
