...
def get_dashboard_kpi(db: Session, acc_cd: str = None) -> dict:
    from database import AccountDailyBalance
    today = datetime.now()
    ...
    total_principal = 0
    net_deposit_today = 0
    net_deposit_this_month = 0
    net_deposit_this_year = 0
    ...
    for tx in all_cash:
        ...
        tx_date_str = tx.dt_cash.strftime("%Y-%m-%d")
        
        if tx_date_str > yesterday_str:
            net_deposit_today += amt
            
        if tx_date_str > last_month_end_str:
            net_deposit_this_month += amt
            
        if tx_date_str > last_year_end_str:
            net_deposit_this_year += amt
    ...
    def calc_period(a_start, a_end):
        if a_start == 0:
            # 기초 자산이 0일 경우, 기준이 없으므로 수익률은 0.0, 수익금은 전체 자산으로 간주하거나 0으로 둠
            # 여기서는 수익금을 현재 자산으로 봄
            profit = a_end
            return_rate = 0.0
        else:
