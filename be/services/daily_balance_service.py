...
def sync_daily_balances_for_account(db: Session, acc_cd: str):
    ...
    if not start_date_str:
        ...
        first_tx = db.query(Transaction.dt_trade).filter(Transaction.acc_cd == acc_cd, Transaction.dt_deleted.is_(None)).order_by(Transaction.dt_trade.asc()).first()
        first_ctx = db.query(TransactionCash.dt_cash).filter(TransactionCash.acc_cd == acc_cd, TransactionCash.dt_deleted.is_(None)).order_by(TransactionCash.dt_cash.asc()).first()
        
        first_dates = []
        if first_tx: first_dates.append(first_tx[0].date())
        if first_ctx: first_dates.append(first_ctx[0].date())
    ...
    for code in unique_stocks:
        try:
            ...
            count = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == code, StockOHLCVCache.trade_date >= start_date_str, StockOHLCVCache.trade_date <= end_date_str).count()
            if count < len(trading_days):
                df_stock = pykrx_stock.get_market_ohlcv(start_compact, end_compact, code)
                for ts, row in df_stock.iterrows():
                    t_date_str = ts.strftime("%Y-%m-%d")
                    close_p = int(row['종가'])
                    if close_p > 0:
                        existing = db.query(StockOHLCVCache).filter(StockOHLCVCache.stock_code == code, StockOHLCVCache.trade_date == t_date_str).first()
                        if existing:
                            existing.close_price = close_p
                        else:
                            db.add(StockOHLCVCache(stock_code=code, trade_date=t_date_str, close_price=close_p, open_price=int(row['시가']), high_price=int(row['고가']), low_price=int(row['저가']), volume=int(row['거래량'])))
                db.commit()
        except Exception as e:
    ...
    for t_date in trading_days:
        ...
        while event_idx < total_events and events[event_idx]["time"] <= t_datetime_end:
            ...
            if event["type"] == "CASH":
                ...
                if tx.trade_type == TradeType.BUY:
                    ...
                    cash_balance += (cost - t_fee)
                    if code in holdings:
                        holdings[code]["quantity"] = max(0, holdings[code]["quantity"] - qty)
            event_idx += 1
        ...
        for code, info in holdings.items():
            ...
            latest_cache = (
                ...
                .filter(StockOHLCVCache.stock_code == code, StockOHLCVCache.trade_date <= t_date)
        ...
        if total_principal > 0:
            return_rate = round(((total_balance - total_principal) / total_principal) * 100, 2)
    ...
    return {"status": "success", "message": f"Synced {len(new_balances)} daily balances up to {end_date_str}."}
