...
## 작업 내용
최근 주식 거래 내역(`LatestTrStockList`)과 유사한 스타일의 **최근 입출금 내역 카드(`LatestTrCash`)**를 생성하여 대시보드에 배치합니다.
...
3. **대시보드 레이아웃 수정**:
   - `Dashboard.tsx`에서 '최근 거래 내역' 카드 우측에 '최근 입출금 내역' 카드를 배치합니다. (1열: 주식 거래 내역, 2열: 입출금 내역)
