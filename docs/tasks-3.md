...
*   **자가 치유 엔진**: 과거 날짜의 자산 변동(매매 또는 현금 원장 추가/수정/삭제)이 일어날 시 발생할 장부 왜곡을 방기하기 위해, 해당 계좌의 과거 기존 시계열 스냅샷 전체를 완전히 Purge하고 연대기순으로 자동 재산출하여 재생성(Purge & Rebuild)하는 트리거를 구축합니다.
