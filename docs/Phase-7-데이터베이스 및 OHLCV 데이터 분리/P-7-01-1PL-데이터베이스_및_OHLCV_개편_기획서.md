...
- **`stock_ohlcv_current`**: 네이버 실시간 API 응답 구조에 맞게 테이블 스키마를 구성합니다.
  ...
  - 기존 `fluctuation_rate` 대신 네이버 API 응답 명칭에 맞춰 **`fluctuation_rate`**(등락률, cr)와 **`change_value`**(대비/전일비, cv) 컬럼을 **신규 추가**하여 저장합니다.
...
- [ ] 신규 테이블 `stock_ohlcv_current` 생성 스크립트 작성 (`dt_updated`, `fluctuation_rate`, `change_value` 포함 / `trading_value`, `fluctuation_rate` 제외).
...
- [ ] **API 응답 매핑 수정**: 
  ...
  - `_current` 수집 로직은 네이버 API의 `cr` 필드를 `fluctuation_rate`에, `cv` 필드를 `change_value`에 매핑하여 저장.
