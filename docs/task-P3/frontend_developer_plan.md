...
## 1. 핵심 스택 & 추가 패키지 구성 계획

현대적이고 견고한 엔터프라이즈급 UI 경험과 타입 안정성 구현을 위해 아래 3가지 패키지를 의존성 목록에 정식 편입합니다.

| 분류 | 패키지 명칭 (Library) | 권장 버전 | 용도 및 적용 범위 |
| :--- | :--- | :--- | :--- |
| **타입 시스템** | **`typescript`** | `^5.3.3` | API 스키마 타입 안정성 강제 및 빌드 타임 컴파일 검증 |
| **벡터 아이콘** | **`@fortawesome/fontawesome-free`** | `^6.5.1` | 사이드바 메뉴, 계좌 설정, 입출금 상태, 피드백 평점 등 전역 아이콘 수록 |
| **드래그앤드롭** | **`sortablejs`** | `^1.15.2` | 대시보드 카드 순서 드래그 정렬 및 설정 화면의 계좌 순서 정렬 연동 |

---

## 2. 프리미엄 메인 대시보드 (Dashboard) 룩앤필 구현 계획

메인 대시보드 화면(`Dashboard.jsx`)은 미려하고 세련된 시각적 레이아웃과 고도로 동적인 인터랙션 카드를 통해 프리미엄 모니터링 환경을 제공합니다. (Mockup 이미지 레이아웃 100% 반영)
...
*   **종목 선택 시**: 해당 종목의 과거 OHLCV 가격 데이터 수집 규격을 기반으로 ApexCharts **캔들 차트 (Candlestick Chart)**를 미려한 디자인으로 렌더링합니다.
*   **계좌 선택 시**: 해당 계좌의 날짜별 자산 잔고 추이 및 수익률 역사를 선형/면적형 차트(**Line/Area Chart**)로 부드러운 애니메이션과 함께 드로잉합니다.

### 5) 푸터 카드 (2개 카드로 단일화 구성)
그리드 하단에는 3개가 아닌 **2개의 카드**를 나란히 배치하여 화면 균형을 유지합니다.
*   **푸터 카드 (1): 최근 주식 매매 내역**: 
