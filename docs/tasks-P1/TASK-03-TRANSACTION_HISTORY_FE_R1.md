# TASK-03: SQLite DB 기반 매수/매도 매매 내역  및 종목코드 자동 검색 UI 구현 (_FE_R1)
...
## 📌 [COMMON] 공통 요구사항
- 매매 내역  수동 등록 시 종목코드를 직접 입력하지 않고, **[종목명 기반 종목코드 자동 검색]** 인터페이스를 구축하여 UX를 개선한다.
...
- **매매 내역  리스트:** `DataTable`을 활용해 내역을 리팅하고 `BUY`는 Red 톤, `SELL`은 Blue 톤 텍스트로 가시성을 확보하라.
